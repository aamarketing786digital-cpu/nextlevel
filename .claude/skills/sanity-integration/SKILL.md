...
```
[A2] Setup → [A2] Images → [B1] Schema → [B1] Queries → [B1] API → [B2] ISR → [B2] SEO → [B2] Live → [B2] Automation
```
...
## Phase 10: Advanced Content Automation & Relationships [B2]

**Prerequisites**: Phase 5-8 complete
**Estimated Time**: 90 minutes
**Cognitive Load**: High (5 concepts)

### Learning Objective
Implement automated content enrichment and complex relationship logic between content types.

### Success Criteria
- [ ] Implement bulk enrichment script using Sanity Client (`patch().commit()`)
- [ ] Create shared TypeScript interfaces for Sanity documents
- [ ] Build premium navigation with dynamic category dropdowns
- [ ] Implement "Related Items" fallback logic for balanced UI
- [ ] Synchronize desktop and mobile navigation schemas

### 1. Bulk Content Enrichment Script
Use for programmatically updating fields based on external data or rule-based generation.

**Template**: `templates/scripts/bulk-enrichment.js`
...
for (const product of products) {
  await client.patch(product._id)
    .set({ specifications: [/* new data */] })
    .commit()
}
```

### 2. Premium Navigation (Category Dropdowns)
Inject category data into the header to improve product discoverability.

**Components**:
- `templates/components/HeaderNavDropdown.tsx`
- `templates/components/MobileNavCollapsible.tsx`

### 3. Related Products Fallback
Ensures the UI doesn't look empty by falling back to other categories if a specific category has few items.

**Logic**:
```typescript
let relatedProducts = await getByCategory(currentCat)
if (relatedProducts.length < 3) {
  const others = await getAllProducts(limit: 3 - relatedProducts.length)
  relatedProducts = [...relatedProducts, ...others]
}
```

### Assessment
- [ ] Script successfully updates multiple documents
- [ ] Dropdown renders correctly on both Mobile & Desktop
- [ ] "Related Products" section always shows 3+ items
- [ ] Shared types resolve all linting errors
...
- **NEW: Advanced Content Automation & Relationships** (Phase 10)
- **NEW: Bulk Enrichment Template** (`templates/scripts/bulk-enrichment.js`)
- **NEW: Shared Sanity Types** (`templates/lib/sanity-types.ts`)
- **NEW: Premium Navigation Templates** (Desktop/Mobile dropdowns)
- **NEW: Related Content Fallback Pattern** (`templates/pages/ProductDetailWithRelated.tsx`)
- **NEW: Advanced Product Schema** (`templates/schemas/advancedProductType.ts`)
